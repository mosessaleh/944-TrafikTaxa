...
async function getDriverScheduleEligibility(prisma, driverId, options = {}) {
  ...
  if (!templateRows.some((row) => row.isActive)) {
    const result = {
      ...
      eligible: false,
      reason: 'NO_TEMPLATE',
      reasonMessage: reasonToMessage('NO_TEMPLATE'),
      metrics: null
    };
    setCachedEligibility(driverId, strict, now, result);
    return result;
  ...
  const insideSchedule = isNowInsideIntervals(now, intervals);
  ...
  if (metrics.dailyWorkedMinutes >= preferences.maxDailyMinutes) {
    const result = {
      ...baseResult,
      eligible: false,
      reason: 'DAILY_LIMIT',
