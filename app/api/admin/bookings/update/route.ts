...
export async function POST(req: NextRequest){
  ...
  try{
    ...
    if (action==='DELIVERED') {
      console.log(`🚚 Starting DELIVERED action for ride ${id}`);
      try {
        ...
        console.log(`✅ DELIVERED action completed successfully for ride ${id}`);
    ...
    if (action==='MARK_PAID') {
      try {
        console.log(`🔄 Starting transaction for DELIVERED action`);
    ...
    return NextResponse.json({ ok:true, ride: finalRideData || updatedRide });
  }catch(e:any){
